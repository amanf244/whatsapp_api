...
const sessionsDir = (sessionId = '') => {
    return join(directory, 'sessions', sessionId ? sessionId : '')
}
...
const createSession = async (sessionId, isLegacy = false, res = null) => {
    console.log(isLegacy ? 'Creating legacy session' : 'Creating modern session');
    ...
    wa.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            setTimeout(
                ...
                statusCode === DisconnectReason.restartRequired ? 0 : parseInt('5000' ?? '0')
            )
        }
